...
class Undistort:
    ...
    @staticmethod
    def points_multi(keypoints, K, dist):
        assert len(keypoints.shape) == 3, keypoints.shape
        shape = keypoints.shape
        flat = keypoints.reshape(-1, 3)
        flat = Undistort.points(flat, K, dist)
    ...
    @staticmethod
    def bboxes(bbox, K, dist):
        keypoints = np.vstack([
            np.stack([bbox[:, 0], bbox[:, 1], np.ones_like(bbox[:, 0])], axis=1),
            np.stack([bbox[:, 2], bbox[:, 3], np.ones_like(bbox[:, 0])], axis=1)
        ])
        kpts = Undistort.points(keypoints, K, dist)
        bbox = np.stack([kpts[:bbox.shape[0], 0], kpts[:bbox.shape[0], 1], kpts[bbox.shape[0]:, 0], kpts[bbox.shape[0]:, 1], bbox[:, 4]], axis=1)
        return bbox

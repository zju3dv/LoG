	...
	const bool debug)
{
	...
	// the per pixel accumulation sample  set as [0,10) with footstep 0.02
	// the accumulatio  record the accumulation slope
	int accumulation_length = 10;
	int accum_step = static_cast<int>(accumulation_length / 0.05);
	printf("accumulation_step %d\n", accum_step);
	torch::Tensor pixel_alpha_accumulation = torch::full({H, W, accum_step}, 0, means3D.options().dtype(torch::kFloat32));
	printf("Allocated pixel_alpha_accumulation memory \n");
	int rendered = 0;
	if (P != 0)
	{
		int M = 0;
		...
		rendered = CudaRasterizer::Rasterizer::forward_sample(
			...
			primitive_index,
			pixel_alpha_accumulation.contiguous().data<float>(),
			debug);
	}
	//   printf("END of SampleGaussiansCUDA\n");
	return std::make_tuple(rendered, out_color, out_point_id, out_point_weight_pixel, out_point_weight, radii, range_size, ranges, primitive_index, pixel_alpha_accumulation);
}

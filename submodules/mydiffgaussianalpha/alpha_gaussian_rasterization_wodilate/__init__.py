...
class _SampleGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                num_rendered, color, point_id, point_weight_pixel, point_weight, radii = _C.rasterize_gaussians(*args)
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw.dump")
                print("\nAn error occured in forward. Please forward snapshot_fw.dump for debugging.")
                raise ex
...
class GaussianRasterizer(nn.Module):
    ...
    def compute_radius(self, positions, scales, rotations):
        # print("Message from wodilate: position shape:",positions.shape)
        with torch.no_grad():

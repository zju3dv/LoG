...
// Run once per instanced (duplicated) Gaussian ID.
// range is set as [)
// compute the range size and the ranges idx
__global__ void compute_range_size(int L,int* range_size , uint2* ranges)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= L)
		return;
	...
	// Read tile ID from key. Update start/end of tile range if at limit.
	uint32_t currtile = idx;
	range_size[currtile]=(int)(ranges[currtile].y - ranges[currtile].x);
}


// __global__ void copy_primitive_index(int L, int* index,uint32_t* point_list)
// {
// 	auto idx = cg::this_grid().thread_rank();
// 	if (idx >= L)
// 		return;
...
// 	uint32_t curidx = idx;
// 	if (idx<10){
// 		printf("Idx:%d,point_list[curidx]:%d",curidx,point_list[curidx]);
// 	}
	
// 	index[curidx]=(int)(point_list[curidx]);
// }

__global__ void copy_primitive_index(size_t L, int* index,uint32_t* point_list)
{
	// auto idx = cg::this_grid().thread_rank();
	...
	// uint32_t curtile = idx;
	index[idx]=static_cast<int>(point_list[idx]);
}

__global__ void copy_ranges(int L, int* ranges,uint2* imgstate_ranges)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= L)
		return;
	...
	uint32_t curtile = idx;
	ranges[curtile*2]=(int)(imgstate_ranges[curtile].x);
	ranges[curtile*2+1]=(int)(imgstate_ranges[curtile].y);
}
...
// Forward rendering procedure for differentiable rasterization
// of Gaussians.
int CudaRasterizer::Rasterizer::forward_sample(
	std::function<char* (size_t)> geometryBuffer,
	std::function<char* (size_t)> binningBuffer,
	std::function<char* (size_t)> imageBuffer,
	const int P, int D, int M,
	...
	bool debug)
{
	...
	// primitive_index= torch::full({num_rendered}, 12, torch::kInt32);
